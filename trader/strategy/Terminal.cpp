...
void Terminal::printOrder(const OrderBase& order, const std::string& type) {
    ...
    std::string format = "%s %s " + formatQuantity + " %s for " + formatPrice + " %s";
    Logger::info(format.c_str(), type.c_str(), order.side() == OrderSide::Buy ? "buy" : "sell", order.baseQuantity(), order.symbol().baseAsset().c_str(), order.price(), order.symbol().quoteAsset().c_str());
}
